...
static u16 GetAdjustedInitialTransitionFlags(struct InitialPlayerAvatarState *, u16, u8);
...
if (I_VS_SEEKER_CHARGING != 0)
    ...
    ApplyWeatherColorMapToPal(NUM_PALS_IN_PRIMARY, NUM_PALS_TOTAL - NUM_PALS_IN_PRIMARY);
...
static void TransitionMapMusic(void)
{
    if (FlagGet(FLAG_DONT_TRANSITION_MUSIC) != TRUE)
    {
        ...
        {
            if (currentMusic == MUS_UNDERWATER || currentMusic == MUS_PS_SURFING)
                return;
            if (TestPlayerAvatarFlags(PLAYER_AVATAR_FLAG_SURFING))
                newMusic = MUS_PS_SURFING;
        }
